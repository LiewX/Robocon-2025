...
# Create a socket object to listen for connections
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    s.bind((HOST, PORT))  # Bind the server to the specified IP and port
    ...
    with conn:
        ...
        # Loop to receive images continuously
        while True:
            # Receive the length of the incoming image data (4 bytes)
            data_len = conn.recv(4)
            if not data_len:
                break  # No data, exit loop
            
            # Convert the data length from bytes to integer
            length = int.from_bytes(data_len, 'big')
            
            # Receive the actual image data (based on the length)
            image_data = b""
            while len(image_data) < length:
                remaining_data = conn.recv(length - len(image_data))
